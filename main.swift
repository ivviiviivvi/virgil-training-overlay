...
func getSanitizedAppName(_ name: String?) -> String {
    // Optimization: Limit input size to prevent DoS and cap processing time.
    let safeName = (name ?? "<none>").prefix(128)

    // Security: Remove control characters to prevent log injection vulnerabilities.
    // Optimization: Use unicodeScalars filtering to avoid allocation overhead of splitting and joining strings.
    // This reduces memory pressure and CPU usage for string processing.
    var result = ""
